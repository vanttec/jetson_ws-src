...
int main(int argc, char *argv[])
{
  ...
  while (ros::ok())
  {
    ...
    Vector2f g;
    g << (1 / (m - X_u_dot)), (1 / (Iz - Y_v_dot));

    Vector2f f;
    f << (((m - Y_v_dot)*v*r + (Xuu*u_abs + Xu*u)) / (m - X_u_dot)), (((- X_u_dot - Y_v_dot)*u*v + (Nr*r)) / (Iz - Y_v_dot));

    u_line = (0.01)*(u + u_last)/2 + u_line;
    u_last = u;

    u_d_line = (0.01)*(u_d + u_d_last)/2 + u_d_line;
    ...
    T(0) = (f(0) - lambda(0)*(zeta_dot(0) - dzeta_dot(0)) + ua(0));
    T(1) = (f(0) - lambda(1)*(zeta_dot(1) - dzeta_dot(1)) + ua(1));
    T(0) = T(0) / g(0);
    T(1) = T(1) / g(1);
